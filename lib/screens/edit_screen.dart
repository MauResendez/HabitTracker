...
class _EditScreenState extends State<EditScreen> {
  ...
  initializeHabit() async {
    DocumentSnapshot currentHabit =
        await FirebaseFirestore.instance.collection('habits').doc(hid).get();
    ...
    setState(() {
      ...
      startTime = TimeOfDay(
          hour: int.parse(
              DateFormat("HH:mm").format(formatStartTime).split(":")[0]),
          minute: int.parse(
              DateFormat("HH:mm").format(formatStartTime).split(":")[1]));
      endTime = TimeOfDay(
          hour: int.parse(
              DateFormat("HH:mm").format(formatEndTime).split(":")[0]),
          minute: int.parse(
              DateFormat("HH:mm").format(formatEndTime).split(":")[1]));
    });
  }
