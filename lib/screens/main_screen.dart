...
class _MainScreenState extends State<MainScreen> {
  ...
  void startHabit() async {
    currentHabit = await FirebaseFirestore.instance
        .collection('habits')
        .where('UserID', isEqualTo: uid)
        .where("Days.${DateFormat('EEEE').format(DateTime.now())}",
            isEqualTo: true)
        .where('startTime', isEqualTo: TimeOfDay.now().format(context))
        .get();
    ...
    if (currentHabit != null) {
      ...
      FirebaseFirestore.instance
          ...
          .doc(currentHabitID)
          .update({
        "Attempts": currentHabit.docs[0]["Attempts"] + 1,
        "inProgress": true
      });
    }
  }
